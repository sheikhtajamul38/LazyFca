...
class FcaClassifier():
    ...
    def binarize_X(self,X:pd.DataFrame,partitions = 5):
    ...
    def make_extent(self,example):
       
        return example[example==1].index.tolist()

    def intersection(self,example,context):
    ...
    def fitClassifier(self,X,y):
        ...
        self.minus = X[y==0]
        if self.random:
            sample_size = int(self.sample_share * self.plus.shape[0])
            self.plus = self.plus.sample(n=sample_size, random_state=self.random_seed)
            self.minus = self.minus.sample(n=sample_size, random_state=self.random_seed)
        self.plus_obj = {}
        self.minus_obj = {}
        pos = plus
        neg = minus
        for col in X.columns:
            self.plus_obj[col] = pos[col][pos[col] == 1].index
            self.minus_obj[col] = neg[col][neg[col] == 1].index
    ...
    def predict(self,X):
        ...
        predictions = []
        for i in range(X.shape[0]):
            extent = self.make_extent(X.iloc[i])
            pos = self.check_hypothesis(extent,'plus')
            neg = self.check_hypothesis(extent,'minus')
            ...
            if pos == neg:
                ...
                prediction = pos>=neg
            predictions.append(self.binary_class[prediction])
        return predictions
    def check_hypothesis(self,extent,base_context):
        sample = (self.plus if base_context =='plus' else self.minus)
        review_sample = (self.minus if base_context =='plus' else self.plus)
        object = (self.minus_obj if base_context == 'plus' else self.plus_obj)

        hyp_score = 0
        for _, i in sample.iterrows():
            intersect = self.intersection(extent,self.make_extent(i))
            k = 0
            if intersect:
                hypothesis = object[intersect[0]]
                for column in intersect:
                    hypothesis = self.intersection(hypothesis,object[column])
                    if not hypothesis:
                        break
                    k = len(hypothesis)/review_sample.shape[0]
                    if k < threshold:
